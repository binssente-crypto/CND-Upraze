          ...
          <>
            <div className="p-6 border-b border-dark-border flex justify-between items-center bg-white/5">
              ...
              <div className="flex gap-3">
                {activeThread.status !== 'in_progress' && activeThread.status !== 'closed' && (
              ...
              {activeThread.messages?.map((msg, idx) => {
                ...
                return (
                  <motion.div key={msg.id || idx} initial={{ opacity: 0, x: isAdmin ? 20 : -20 }} animate={{ opacity: 1, x: 0 }} className={`flex gap-6 ${isAdmin ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`max-w-[65%] space-y-2 ${isAdmin ? 'text-right' : ''}`}>
                      ...
                      <div className={`p-5 rounded-3xl text-sm leading-relaxed shadow-xl ${isAdmin ? 'bg-primary-600 text-white rounded-tr-none' : 'bg-dark-card border border-dark-border rounded-tl-none text-gray-200'}`}>
                        {msg.message}
                      </div>
                    </div>
                  </motion.div>
